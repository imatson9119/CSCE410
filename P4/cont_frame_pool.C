                             ...
                             unsigned long _n_info_frames)
{
    ...
    // unspecified
    if(info_frame_no == 0){
	n_info_frames = this->needed_info_frames(nframes);
	nFreeFrames -= n_info_frames;
        bitmap = (unsigned char *) (base_frame_no * FRAME_SIZE);	
    } else {
	// make sure that the provided frames aren't in our allocated segment
	assert(info_frame_no + n_info_frames < base_frame_no 
		|| info_frame_no >= base_frame_no + nframes);
    ...
    //Console::puts("There are ");Console::putui(nframes);Console::puts(" frames and we need ");Console::putui(n_info_frames);Console::puts(" info frames\n");
    if(info_frame_no == 0){
	// First frame will always be used
	// 10 represents a head of allocated segment, 11, is free, 00 is allocated but not head

	// If there are any remaining info frames, set them to inaccessible (01)
	for(int i = 0; i < n_info_frames; i++){
    	    bitmap[i/4] &= ~(0x80 >> i%4 * 2);
    	    bitmap[i/4] |= (0x80 >> i%4 * 2 + 1);
	    Console::puts("\nFRAME ");Console::putui(base_frame_no + i);Console::puts(" IS AN INFO FRAME");
	}
    } else{
...
unsigned long ContFramePool::get_frames(unsigned int _n_frames)
{
	...
	if(bitmap[i/4] == 0x0){
	    // Entire character empty-- skipping ahead
	    count = 0;
	    i+= 3;
	}
	// If the current frame is empty
	else if(bitmap[i/4] & (0x80 >> i%4 * 2) && bitmap[i/4] & (0x80 >> i%4 * 2 + 1)){
	    count++;
                                      ...
                                      unsigned long _n_frames)
{
    ...
    // Set the frames to inaccessible (01)
    for(unsigned long i = _base_frame_no - base_frame_no; i < _base_frame_no-base_frame_no+_n_frames; i++){
	
	if(i%4 == 0 && !(bitmap[i/4] & (0x80 >> i%4 * 2 +1) > 0)){
	   Console::putui(bitmap[i/4]);
	}	
	// Is the frame actually free?
	assert(bitmap[i/4] & (0x80 >> i%4 * 2 +1) > 0);
    	bitmap[i/4] &= ~(0x80 >> i%4 * 2);
    	bitmap[i/4] |= (0x80 >> i%4 * 2 + 1);
    	nFreeFrames--;
    }
...
{
    return  (_n_frames-1) / (FRAME_SIZE * 4) + 1;
}
